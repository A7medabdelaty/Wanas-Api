...
namespace Wanas.API.RealTime
{
    public class RealTimeNotifier : IRealTimeNotifier
    {
        ...
        {
            await _hub.Clients.Group($"chat_{chatId}").SendAsync("ParticipantAdded", chatId, userId);
        }

        public async Task NotifyParticipantRemovedAsync(int chatId, string userId)
        {
            await _hub.Clients.Group($"chat_{chatId}").SendAsync("ParticipantRemoved", chatId, userId);
        }

        public async Task NotifyMessageReceivedAsync(MessageDto message)
        {
            await _hub.Clients.Group($"chat_{message.ChatId}").SendAsync("ReceiveMessage", message);
        }

        public async Task NotifyMessageDeletedAsync(int chatId, int messageId)
        {
            await _hub.Clients.Group($"chat_{chatId}").SendAsync("MessageDeleted", chatId, messageId);
        }

        public async Task NotifyMessageReadAsync(int chatId, int messageId, string userId)
        {
            await _hub.Clients.Group($"chat_{chatId}").SendAsync("MessageRead", chatId, messageId, userId);
        }
    }
}
